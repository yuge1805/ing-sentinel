...
//        rule1.setMaxQueueingTimeMs(20 * 1000);
    ...
    private static void scheduledSimulate() {
        ScheduledExecutorService scheduledExecutorService = Executors.newScheduledThreadPool(1);
        scheduledExecutorService.scheduleAtFixedRate(() -> {
            simulatePulseFlowNew();
        }, 0, 10, TimeUnit.MILLISECONDS);
    }

    private static void simulatePulseFlowNew() {
        CyclicBarrier cyclicBarrier = new CyclicBarrier(requestQps);
        for (int i = 0; i < requestQps; i++) {
            Thread thread = new Thread(new Runnable() {
                @Override
                public void run() {
